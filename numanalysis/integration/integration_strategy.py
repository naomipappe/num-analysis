...
class AprioriEstimationStrategy(IntegrationStrategy):
    ...
    def calculate(cls, integrand: Callable[[float], float], quadratic_formula: Type[QuadraticFormula],
        ...
        integral = quadratic_formula.value_composite(
            integration_borders, integrand, step)
    ...
    def apriori_estimation(cls, integration_borders: tuple, quadratic_formula: Type[QuadraticFormula],
        ...
        values = vectorize(integrand_nth_derivative)(
            linspace(*integration_borders, 10 ** 5))
        dnf_measure = max(abs(values))
        error, step = quadratic_formula.quadratic_formula_step_error(
            integration_borders, dnf_measure, tolerance)
        return error, step
...
class RungeStrategy(IntegrationStrategy):
    ...
    def calculate(cls, integrand: Callable[[float], float], quadratic_formula: Type[QuadraticFormula],
        ...
        integral_prev = quadratic_formula.value_composite(
            integration_borders, integrand, step)
        step = step / 2
        integral_next = quadratic_formula.value_composite(
            integration_borders, integrand, step)
        ...
        integral_next = cls._richardson_clarification(
            integral_next, integral_prev, algebraic_precision)
...
class AdaptiveStrategy(IntegrationStrategy):
    ...
    def calculate(cls, integrand: Callable[[float], float], quadratic_formula: Type[QuadraticFormula],
        ...
        adaptive_steps = []
        integral_next, integral_prev = cls.adaptive_iteration(
            integrand, left_border, quadratic_formula, step)
    ...
    def adaptive_iteration(cls, integrand, left_border, quadratic_formula, step):
        integral_prev = quadratic_formula.value_simple(
            (left_border, left_border + step), integrand)
        integral_next = quadratic_formula.value_simple((left_border, left_border + step / 2), integrand) + \
            quadratic_formula.value_simple(
                (left_border + step / 2, left_border + step), integrand)
