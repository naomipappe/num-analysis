    ...
    def __init__(self, borders: tuple, alpha: float, beta: float, gamma: float, delta: float, k, mu_1, mu_2, variable):
        ...
        if self._mu_1() == 0 and self._mu_2() == 0:
            ...
            self._A_psi = (self._mu_1() - self._mu_2() * beta / delta) / (
                -self._k(self._border_left) + beta *
                self._border_left - (beta / delta)
...
class BasisFunction(FunctionalSystem):
    ...
    def get_function(self, j):
        if j == 0:
            return ((self.variable - self._border_left) ** 2) * (self.variable - self._C)
        elif j == 1:
            return ((self._border_right - self.variable) ** 2) * (self.variable - self._D)
        return ((self.variable - self._border_left) ** 2) * ((self._border_right - self.variable) ** (j-1))
...
class AnotherSystem(FunctionalSystem):
    ...
    def get_function(self, j: int):
        ...
        return (self._variable - self._border_left) ** 2 * ((self._border_right - self._variable) ** (j-1))
