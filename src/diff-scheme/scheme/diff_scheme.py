...
class FiniteElementDiffScheme:
    ...
    def solve(self, n: int):
        ...
        def approximation(x: float):
            return sum([coefficients[i] * self.__phi(i, x) for i in range(n + 1)])
...
class MonotonicScheme:
    ...
    def _build_system(self, n, equation_rhs):
        def call_equation_rhs(x: float) -> float:
            return lambdify('x', equation_rhs, 'numpy')(x)
    ...
    def b_pos(self, i: int) -> float:
        return self._r_pos(i) / self._k(self._nodes[i])

    def b_neg(self, i: int) -> float:
        return self._r_neg(i) / self._k(self._nodes[i])

    def _r_pos(self, i: int) -> float:
        return (self._p(self._nodes[i]) + abs(self._p(self._nodes[i]))) / 2

    def _r_neg(self, i: int) -> float:
        return (self._p(self._nodes[i]) - abs(self._p(self._nodes[i]))) / 2
